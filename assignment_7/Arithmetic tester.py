# Write grammar-based test case generators in Python
# Use yield in Python to nondeterministally produce values
...
def makeTest(bound):
    if bound <= 0:
        ...
        for left in makeTest(bound - 1):
            for right in makeTest(bound - 1):
...
def makeTestWithNums(bound, input_list):
    if bound <= 0:
        ...
        for left in makeTestWithNums(bound - 1, input_list):
            for right in makeTestWithNums(bound - 1, input_list):
                yield Plus(left, right)
                yield Minus(left, right)
                yield Mult(left, right)
                yield Negate(left)
