...
makeTest(_, number(0)).
makeTest(MaxDepth, plus(Expression1, Expression2)) :-
    MaxDepth > 0,
    NewMaxDepth is MaxDepth - 1,
    makeTest(NewMaxDepth, Expression1),
    makeTest(NewMaxDepth, Expression2).
makeTest(MaxDepth, minus(Expression1, Expression2)) :-
    MaxDepth > 0,
    NewMaxDepth is MaxDepth - 1,
    makeTest(NewMaxDepth, Expression1),
    makeTest(NewMaxDepth, Expression2).
makeTest(MaxDepth, mult(Expression1, Expression2)) :-
    MaxDepth > 0,
    NewMaxDepth is MaxDepth - 1,
    makeTest(NewMaxDepth, Expression1),
    makeTest(NewMaxDepth, Expression2).
makeTest(MaxDepth, negate(Expression)) :-
    MaxDepth > 0,
    NewMaxDepth is MaxDepth - 1,
    makeTest(NewMaxDepth, Expression).
...
% T = negate(number(2)) ;
% T = negate(number(3)) .
makeTestWithNums(0, Numbers, number(Number)) :-
    member(Number, Numbers).
makeTestWithNums(MaxDepth, Numbers, AST) :-
    MaxDepth > 0,
    NewMaxDepth is MaxDepth - 1,
    makeTestWithNums(NewMaxDepth, Numbers, AST). 
makeTestWithNums(MaxDepth, Numbers, AST) :-
    MaxDepth > 0,
    NewMaxDepth is MaxDepth - 1,
    (
        makeTestWithNums(NewMaxDepth, Numbers, Expression1),
        makeTestWithNums(NewMaxDepth, Numbers, Expression2),
    ...
    ;
        makeTestWithNums(NewMaxDepth, Numbers, Expression1),
        makeTestWithNums(NewMaxDepth, Numbers, Expression2),
        AST = mult(Expression1, Expression2)
    ;
        makeTestWithNums(NewMaxDepth, Numbers, Expression),
        AST = negate(Expression)
    ).
